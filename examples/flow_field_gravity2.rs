// Straight flow field implementation with two distinct changes:
// 1. The field is defined by a potential field based on gravitational forces.
// 2. The start points are not arranged in a grid, but rather around the "gravitational bodies"
//
// The gravitational field is defined by the GravitySystem struct in the util/gravity module.
// The algorithm for the field is:
// 1. Create random gravitational bodies with mass and position (x,y)
// 2. For each body, calculate the component (x,y) forces applied to a point mass.
//    Be sure to normalize the force vectors.
// 3. Sum up all the forces acting on a point mass
// 4. Apply the force to the point (i.e. add the force to the current position)
//
// cargo run --release --example flow_field_gravity2 -- --loops 10
extern crate chrono;
extern crate nannou;

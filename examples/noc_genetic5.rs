...
impl DNA {
  fn new(size: usize, pos: Point2) -> Self {
    // random_range(32, 128).into()
    // random_ascii()
    let genes = (0..size).map(|_| Self::random_value()).collect();
  ...
  fn calc_fitness(mut self, target: &Vec<(Point2, Rgb)>) -> Self {
    // kind of a weird way of finding the closest color... we'll see if it works
    let mut closest_color = 0;
    for (i, (pt, color)) in target.iter().enumerate() {
      if pt.distance(self.pos) < target[closest_color].0.distance(self.pos) {
        closest_color = i;
      }
    }

    let closest_color = target[closest_color].1;
    // closer to the target number is better
    let r_fitness = 1.0 - (self.genes[0] - closest_color.red).abs();
    let g_fitness = 1.0 - (self.genes[1] - closest_color.green).abs();
    let b_fitness = 1.0 - (self.genes[2] - closest_color.blue).abs();
  ...
  fn crossover(&self, other: &Self) -> Self {
    let mut child = DNA::new(self.genes.len(), self.pos.clone());
  ...
  fn mutate(&mut self, mutation_rate: f32) {
    for i in 0..self.genes.len() {
      if random_f32() < mutation_rate {
        self.genes[i] = Self::random_value();
      }
    }
  }
...
impl Population {
  fn new(target: Vec<(Point2, Rgb)>, size: usize, mutation_rate: f32, win: &Rect) -> Self {
    Population {
      population: (0..size)
        // .map(|_| DNA::new(target.len()).calc_fitness(&target))
        .map(|n| {
          let (x, y) = x_y_from_index(n, size / 2, win.left(), win.bottom());
          DNA::new(3, pt2(x, y)).calc_fitness(&target)
        })
        .collect(),
      target,
      mating_pool: Vec::new(),
...
fn model(app: &App) -> Model {
  ...
  let target = random_polygons(&win);
  let population = Population::new(target, population_size, mutation_rate, &win);
...
fn random_polygons(win: &Rect) -> Vec<(Point2, Rgb)> {
  let diag = win.w().hypot(win.h());
  (0..8)
    .map(|n| {
      let angle = map_range(n, 0, 8, 0.0, 2.0 * PI);
      let x = angle.cos() * diag / 4.0;
      let y = angle.sin() * diag / 4.0;
      println!("{}, {}", x, y);
      // hsl(0.5, 0.5, 0.5)
      (
        pt2(x, y),
        rgb(
          random_range(0.0, 255.0) / 255.0,
          random_range(0.0, 255.0) / 255.0,
          random_range(0.0, 255.0) / 255.0,
        ),
      )
...
fn rgb_from_genes(genes: &Vec<f32>) -> Rgb {
  let r = genes[0];
  let g = genes[1];
  let b = genes[2];
  rgb(r, g, b)

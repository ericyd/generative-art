...
export function draw() {
  ...
  const children = array(10).map((i) =>
    path.withAttrs({ d: pathBuilder((d) => {
      d.move(randomPoint(20, 80, 20, 80, rng))
      let i = 0
      while (i < 50) {
        if (i % 15 === 0) {
          d.smoothBezier(randomPoint(20, 80, 20, 80, rng), randomPoint(20, 80, 20, 80, rng), 'absolute')
          // d.smoothBezier(randomPoint(-5, 5, -5, 5, rng), randomPoint(-5, 5, -5, 5, rng), 'relative')
        } else if (i % 3 === 0) {
          d.smoothBezier(randomPoint(-5, 5, -5, 5, rng), randomPoint(-5, 5, -5, 5, rng), 'relative')
        } else {
          d.smoothBezier(randomPoint(-3, 3, -3, 3, rng), randomPoint(-3, 3, -3, 3, rng), 'relative')
        }
        i++
      }
    }), stroke: colors[i % 4] })

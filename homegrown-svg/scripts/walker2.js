...
export function draw() {
  ...
  const path = tag("path", {
    ...
    style: "filter: drop-shadow(1px 1px 0.5px #aaa);",
  });

  const baseColor = { h: 260, s: 25, l: 18 };
  ...
  // This sketch isn't really working out
  // I'm thinking more minimal: maybe a squiggle that is white on white background with drop shadow?
  const nChildren = 300;
  const children = shuffle(array(nChildren), rng).map((i) => {
  ...
  children.push(
    tag("polygon", {
      points: `${w / 3},${h * 2 / 3} ${w / 2},${h / 3} ${w * 2 / 3},${h * 2 / 3}`,
      fill: "rgba(0, 0, 0, 1.5)",

...
function connectContourSegments(contours: Contour[], threshold?: number): Vector2[][] {
  ...
  const contourLinesByThreshold = new Map<number, Vector2[][]>()
  ...
  for (const [layer, segments] of contourMap.entries()) {
    outerLoop: while (segments.length > 0) {
      ...
      innerLoop: while (true) {
        ...
        if (firstMatchingSegmentIndex === -1) {
          if (line.length > 5) {
            if (contourLinesByThreshold.has(layer)) {
              contourLinesByThreshold.get(layer)?.push(line)
            } else {
              contourLinesByThreshold.set(layer, [line])
            }
          }
          break innerLoop;
        }
  ...
  // process contonurLinesByThreshold
  // loop through each segment in the threshold
  // connect to other segments with matching head/tails
  // push result to contourLines
  //// this is "necessary" because it's possible to build two segments on the same threshold
  //// which SHOULD connect, but did not connect originally because they started on opposite ends.
  //// TODO: OHHHHHHHH wow this is IDENTICAL to the above code.
  ///// I think this will be worth it to reduce number of contours though.
  for (const [layer, segments] of contourLinesByThreshold.entries()) {
    outerLoop: while (segments.length > 0) {
      // grab arbitrary segment
      const line: Vector2[] | undefined = segments.pop()
      if (!line) break outerLoop;

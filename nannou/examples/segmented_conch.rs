...
const CENTER_X: f32 = -115.;
const CENTER_Y: f32 = 200.;
...
fn draw_segments(draw: &Draw, model: &Model) {
  ...
  for n in 0..model.n_segments {
    ...
    let n_layers = 8;
    let mut min_radius = lerp(
      model.min_radius,
      model.max_radius,
      n as f32 / (model.n_segments as f32 / 2.),
    );
    let mut max_radius = lerp(model.min_radius, model.max_radius, factor);
    let mut hue = model.hue;
    for _l in 0..n_layers {
      // this would be nice for nesting
      draw_segment(
        hue,
        model.sat,
        model.lum,
...
fn point_mapper(
  ...
  move |n| {
    ...
    let x = theta.cos() * radius + CENTER_X;
    let y = theta.sin() * radius + CENTER_Y;
    pt2(x, y)
  }
}

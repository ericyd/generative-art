...
fun main() = application {
  ...
  program {
    ...
    extend {
      ...
      val baseLine = FractalizedLine(
        listOf(
          center + Vector2(cos(baseLineOrientation), sin(baseLineOrientation)) * center.length,
          center - Vector2(cos(baseLineOrientation), sin(baseLineOrientation)) * center.length
        ),
        rng
      ...
      val packed = packFixedCirclesControlled(
        bodies = generateBodies(rng, baseLine),
        incremental = DEBUG,
        qtreeBase = qtreeBase,
        rng = rng,
        verbose = VERBOSE
        ...
        layer {
          draw {
            drawer.shadeStyle = LinearGradient(
              color0 = ColorRGBa.fromHex("3C5D99"),
              color1 = ColorRGBa.fromHex("FDA58C"),

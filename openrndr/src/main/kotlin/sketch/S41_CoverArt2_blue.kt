...
fun main() = application {
  ...
  program {
    ...
    /**
     * Offsets a ShapeContour by a vector2.
     ...
     */
    fun offsetContour(contour: ShapeContour, vector2: Vector2): ShapeContour {
      val segments = contour.segments.map { Segment(it.start + vector2, it.end + vector2) }
    ...
    fun explode(contour: ShapeContour, maxRadius: Double, rng: Random = Random.Default): ShapeContour {
      ...
      return offsetContour(contour, explodeVec)
    }
    ...
    extend {
      ...
      val colors = listOf(
        ColorRGBa.fromHex("97E1FC"),
        ColorRGBa.fromHex("77D1F7"),
        ColorRGBa.fromHex("00466B"),
        ColorRGBa.fromHex("3F8FDB"),
        ColorRGBa.fromHex("92BCE0"),
      )
      ...
      drawer.isolatedWithTarget(rt) {
        ...
        //   drawer.fill = shatterFillGradient.index(random(0.0, 1.0, rng)).mix(ColorRGBa.BLACK, 0.25)
        //   drawer.stroke = shatterStrokeGradient.index(random(0.0, 1.0, rng)).mix(ColorRGBa.BLACK, 0.25)
        //   drawer.contour(offsetContour(c, Vector2(cos(PI * 0.25) * scale(5.0), sin(PI * 0.25) * scale(5.0))))
        // }
        ...
        for (c in mappedContours) {
          ...
          drawer.fill = shatterFillGradient.index(random(0.0, 1.0, rng))
          drawer.stroke = shatterStrokeGradient.index(random(0.0, 1.0, rng))
          drawer.contour(c)
        }
      }

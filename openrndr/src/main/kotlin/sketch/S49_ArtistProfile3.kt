...
fun main() = application {
  configure {
//    spotify requirements are 2660x1140 for heading image
//    width = 1330
//    height = 570
    width = 665
    height = 285
...
//         start with a "background" layer
        val bounds = if (i == 0) {
          ...
          Rectangle(
            ...
            y = random(height * -0.05, height * 0.65, rng),
            width = random(width * 0.85, width * 1.25, rng),
        ...
        for (y in (bounds.y + bounds.height).toInt() downTo  bounds.y.toInt() step tileHeight.toInt()) {
          var x = bounds.x
          while (x < bounds.x + bounds.width) {
            // calculate the derivative at this point
            // d/dx sin(x) == cos(x), but our sine is more complex so need to approximate it
            val epsilon = 0.15
            val ddx = (sine(x + epsilon * 0.5) - sine(x - epsilon * 0.5)) / epsilon
